<?php
class Counter_sales_transaction_model extends CI_Model 
{
	var $trans_type = 5;
	var $insert_id = NULL;
	...
	function delete($id)
	{
		...
		$this->db->delete('counter_sales_transaction_items');
	...
	function create($data, $items)
	{
		...
		$this->access->log_insert($id, 'Penjualan Counter');
		$this->db->trans_complete();
		return $this->db->trans_status();
	}// end of function 
	function update($id, $data, $items)
	{
		...
		{			
			...
			$this->db->insert('counter_sales_transaction_items', $row); 
			$index++;
		}
