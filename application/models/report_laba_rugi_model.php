...
class Report_laba_rugi_model extends CI_Model{
	...
	function get_detail($period_id, $coa_id) {
		$query = "select coalesce(case when sum(e_amount_debet - e_amount_kredit) > 0 then sum(e_amount_debet - e_amount_kredit) end,0) as jumlah 
	...
	{
		$sql = "select coalesce(case when sum(e_amount_debet - e_amount_kredit) > 0 then sum(e_amount_debet - e_amount_kredit) else sum(e_amount_kredit - e_amount_debet) end,0) as jumlah
				from trial_balance a
	...
	//	query();
		$result = null;
		foreach ($query->result_array() as $row) $result = format_html($row);
		return array($result['jumlah'], $result['total_credit']);
	}
