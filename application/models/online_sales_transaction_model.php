<?php
class Online_sales_transaction_model extends CI_Model 
{
	var $trans_type = 6;
	var $insert_id = NULL;
	...
	function delete($id)
	{
		...
		$this->db->delete('online_sales_transaction_items');
	...
	function create($data, $items)
	{
		...
		$this->access->log_insert($id, 'Penjualan Online');
		$this->db->trans_complete();
		return $this->db->trans_status();
	}// end of function 
	function update($id, $data, $items)
	{
		...
		{			
			...
			$this->db->insert('online_sales_transaction_items', $row); 
			$index++;
		}
