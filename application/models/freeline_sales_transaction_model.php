<?php
class Freeline_sales_transaction_model extends CI_Model 
{
	var $trans_type = 3;
	var $insert_id = NULL;
	...
	function delete($id)
	{
		...
		$this->db->delete('freeline_sales_transaction_items');
	...
	function create($data, $items, $point)
	{
		$this->db->trans_start();
		...
		$old_point = $this->get_old_point($data['salesman_id']);
		$data_point['salesman_point'] = $old_point + $point;
		$this->db->where('salesman_id', $data['salesman_id']); // data yg mana yang akan di update
		$this->db->update('salesmans', $data_point);
	...
	function update($id, $data, $items)
	{
		...
		{			
			...
			$this->db->insert('freeline_sales_transaction_items', $row); 
			$index++;
		}
	...
	function detail_list_loader($id)
	{
		...
		$this->db->join('product_stocks b', 'b.product_id = a.product_id and price_id = 2');
	...
	function get_old_point($id)
	{
		$sql = "select salesman_point
				from salesmans 
