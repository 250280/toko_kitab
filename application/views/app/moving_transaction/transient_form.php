...
$(function(){
	...
	function load_product_stock(id){
		...
		$.ajax({
			type: 'POST',
			url: '<?=site_url('moving_transaction/load_product_stock')?>',
			data: data,
			dataType: 'json',
			success: function(data){					
				$('input[name="i_product_id"]').val(data.content['product_id']);
				$('input[name="i_product_code"]').val(data.content['product_code']);
				$('input[name="i_product_stock_qty"]').val(data.content['product_stock_qty']);
...
<table class="form_layout">
	<tr>
	 ...
	 <span class="lookup" id="lookup_active_product">
        ...
        <input type="hidden" name="i_product_stock_qty" value="" />
