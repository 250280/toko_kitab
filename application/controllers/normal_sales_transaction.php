...
class Normal_sales_transaction extends CI_Controller 
{	
	function __construct()
	{
		parent::__construct();	
		$this->load->model('normal_sales_transaction_model');
		$this->load->library('render');
		
		// set kode module ini .. misal usr
		$this->access->set_module('transaction.normal_sales_transaction');
	...
	function index()
	{
		...
		$this->render->add_form('app/normal_sales_transaction/form', $data);
		$this->render->build('Transaksi Penjualan User');
		
		$this->render->add_view('app/normal_sales_transaction/transient_list');
		$this->render->build('Data Produk');
		
		$this->render->add_view('app/normal_sales_transaction/form_end', $data);
	...
	{
		$data = $this->normal_sales_transaction_model->list_controller();
		send_json($data); 
	}
	...
	function form($id = 0)
	{
		...
		if ($id == 0) {
			...
			$result = $this->normal_sales_transaction_model->read_id($id);
	...
	function form_action($is_delete = 0) // jika 0, berarti insert atau update, bila 1 berarti delete
	{
		...
		if($is_delete)
		{
			...
			$is_process_error = $this->normal_sales_transaction_model->delete($id);
		...
		{
			$get_purchase_price = $this->normal_sales_transaction_model->get_purchase_price($list_product_id[$key]);
		...
		{ 
			...
			$error = $this->normal_sales_transaction_model->create($data, $items);
			send_json_action($error, "Data telah ditambah", "Data gagal ditambah", $this->normal_sales_transaction_model->insert_id);
		}
		else // id disebutkan, lakukan proses UPDATE
		{
			$error = $this->normal_sales_transaction_model->update($id, $data, $items);
	...
	function detail_list_loader($transaction_id=0)
	{
		...
		$data = $this->normal_sales_transaction_model->detail_list_loader($transaction_id);
		$sort_id = 0;
	...
	{		
		...
		$check_stock = $this->normal_sales_transaction_model->check_stock($product_stock_id);
		
		$get_data_product = $this->normal_sales_transaction_model->get_data_product($product_stock_id);
	...
	function load_product_stock()
	{
		...
		$query = $this->normal_sales_transaction_model->load_product_stock($id);
		$data = array();
	...
	if($id){
	   ...
	   $result = $this->normal_sales_transaction_model->read_id($id);
		...
		$data_detail = $this->normal_sales_transaction_model->get_data_detail($id);
	   
	   $this->global_model->create_report('normal_sales_transaction', 'report/normal_sales_transaction.php', $data, $data_detail, 'header.php');
	}
	}
