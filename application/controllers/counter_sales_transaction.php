...
class Counter_sales_transaction extends CI_Controller 
{	
	function __construct()
	{
		parent::__construct();	
		$this->load->model('counter_sales_transaction_model');
		$this->load->library('render');
		
		// set kode module ini .. misal usr
		$this->access->set_module('transaction.counter_sales_transaction');
	...
	function index()
	{
		...
		$data['row_id'] = '';
		$data['transaction_code']			= format_code('transactions','transaction_code','PC',7);
		$data['stand_id'] = '';
		...
		$this->render->add_form('app/counter_sales_transaction/form', $data);
		$this->render->build('Transaksi Penjualan Counter');
		
		$this->render->add_view('app/counter_sales_transaction/transient_list');
		$this->render->build('Data Produk');
		
		$this->render->add_view('app/counter_sales_transaction/form_end', $data);
		$this->render->build('Pembayaran');
		
		
		$this->render->show('Transaksi Penjualan Counter');
		
	}
	function table_controller()
	{
		$data = $this->counter_sales_transaction_model->list_controller();
		send_json($data); 
	}
	...
	function form($id = 0)
	{
		...
		if ($id == 0) {
			...
			$result = $this->counter_sales_transaction_model->read_id($id);
	...
	function form_action($is_delete = 0) // jika 0, berarti insert atau update, bila 1 berarti delete
	{
		...
		if($is_delete)
		{
			...
			$is_process_error = $this->counter_sales_transaction_model->delete($id);
		...
		{
			$get_purchase_price = $this->counter_sales_transaction_model->get_purchase_price($list_product_id[$key]);
		...
		{ 
			...
			$error = $this->counter_sales_transaction_model->create($data, $items);
			send_json_action($error, "Data telah ditambah", "Data gagal ditambah", $this->counter_sales_transaction_model->insert_id);
		}
		else // id disebutkan, lakukan proses UPDATE
		{
			$error = $this->counter_sales_transaction_model->update($id, $data, $items);
	...
	function detail_list_loader($transaction_id=0)
	{
		...
		$data = $this->counter_sales_transaction_model->detail_list_loader($transaction_id);
		$sort_id = 0;
	...
	{		
		...
		$this->render->add_form('app/counter_sales_transaction/transient_form', $data);
	...
	{		
		...
		$check_stock = $this->counter_sales_transaction_model->check_stock($product_stock_id);
		
		$get_data_product = $this->counter_sales_transaction_model->get_data_product($product_stock_id);
	...
	function load_product_stock()
	{
		...
		$query = $this->counter_sales_transaction_model->load_product_stock($id);
		$data = array();
		...
		foreach($query->result_array() as $row)
		{
			...
			$data['price'] = $row['counter_price'];
			$data['qty'] = 1;
			$data['total'] = $row['counter_price'];
		}
	...
	if($id){
	   ...
	   $result = $this->counter_sales_transaction_model->read_id($id);
		...
		$data_detail = $this->counter_sales_transaction_model->get_data_detail($id);
	   
	   $this->global_model->create_report('counter_sales_transaction', 'report/counter_sales_transaction.php', $data, $data_detail);
	}
	}
}
