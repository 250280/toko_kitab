...
class sales_transaction_report extends CI_Controller 
{	
	function __construct()
	{
		parent::__construct();	
		$this->load->model('sales_transaction_report_model');
		$this->load->library('render');
		
		// set kode module ini .. misal usr
		$this->access->set_module('report.sales_transaction_report');
	...
	function index()
	{
		$this->render->add_view('app/sales_transaction_report/list');
		$this->render->build('Data Transaksi Penjualan');
		$this->render->show('Transaksi Penjualan');
		
	}
	function table_controller()
	{
		$data = $this->sales_transaction_report_model->list_controller();
		send_json($data); 
	}
	...
	function form($id = 0)
	{
		...
		if ($id == 0) {
			...
			$result = $this->sales_transaction_report_model->read_id($id);
		...
		$this->render->add_form('app/sales_transaction_report/form', $data);
		$this->render->build('Penjualan');
		$this->render->add_view('app/sales_transaction_report/transient_list');
		$this->render->build('Data Produk');
		//if($id){
		//$this->access->generate_log_view($id);
		//}
		$this->render->show('Penjualan');
	}
	function form_action($is_delete = 0) // jika 0, berarti insert atau update, bila 1 berarti delete
	{
		...
		if($is_delete)
		{
			...
			$is_process_error = $this->sales_transaction_report_model->delete($id);
		...
		{
			$get_purchase_price = $this->sales_transaction_report_model->get_purchase_price($list_product_id[$key]);
		...
		{ 
			...
			$error = $this->sales_transaction_report_model->create($data, $items);
			send_json_action($error, "Data telah ditambah", "Data gagal ditambah");
		}
		else // id disebutkan, lakukan proses UPDATE
		{
			$error = $this->sales_transaction_report_model->update($id, $data, $items);
	...
	function detail_list_loader($transaction_id=0)
	{
		...
		$data = $this->sales_transaction_report_model->detail_list_loader($transaction_id);
		$sort_id = 0;
	...
	{		
		...
		$this->render->add_form('app/sales_transaction_report/transient_form', $data);
	...
	{		
		...
		$check_stock = $this->sales_transaction_report_model->check_stock($product_stock_id);
		
		$get_data_product = $this->sales_transaction_report_model->get_data_product($product_stock_id);
	...
	function load_product_stock()
	{
		...
		$query = $this->sales_transaction_report_model->load_product_stock($id);
		$data = array();
