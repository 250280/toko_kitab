...
class Freeline_sales_transaction extends CI_Controller 
{	
	function __construct()
	{
		parent::__construct();	
		$this->load->model('freeline_sales_transaction_model');
		$this->load->library('render');
		
		// set kode module ini .. misal usr
		$this->access->set_module('transaction.freeline_sales_transaction');
	...
	function index()
	{
		...
		$data['transaction_code']			= format_code('transactions','transaction_code','PF',7);
		$data['stand_id'] = '';
		...
		$this->render->add_form('app/freeline_sales_transaction/form', $data);
		$this->render->build('Transaksi Penjualan Freeline');
		
		$this->render->add_view('app/freeline_sales_transaction/transient_list');
		$this->render->build('Data Produk');
		
		$this->render->add_view('app/freeline_sales_transaction/form_end', $data);
	...
	{
		$data = $this->freeline_sales_transaction_model->list_controller();
		send_json($data); 
	}
	...
	function form($id = 0)
	{
		...
		if ($id == 0) {
			...
			$result = $this->freeline_sales_transaction_model->read_id($id);
		...
		$this->render->add_form('app/freeline_sales_transaction/form', $data);
		$this->render->build('Penjualan Freeline');
		$this->render->add_view('app/freeline_sales_transaction/transient_list');
		$this->render->build('Data Produk');
		//if($id){
	...
	function form_action($is_delete = 0) // jika 0, berarti insert atau update, bila 1 berarti delete
	{
		...
		if($is_delete)
		{
			...
			$is_process_error = $this->freeline_sales_transaction_model->delete($id);
		...
		$this->form_validation->set_rules('i_salesman_id','Salesman','trim|required');
		...
		$data['salesman_id']				= $this->input->post('i_salesman_id');
		...
		{
			$get_purchase_price = $this->freeline_sales_transaction_model->get_purchase_price($list_product_id[$key]);
			$get_product_point = $this->freeline_sales_transaction_model->get_product_point($list_product_id[$key]);
		...
		{ 
			...
			$error = $this->freeline_sales_transaction_model->create($data, $items, $point);
			send_json_action($error, "Data telah ditambah", "Data gagal ditambah", $this->freeline_sales_transaction_model->insert_id);
		}
		else // id disebutkan, lakukan proses UPDATE
		{
			$error = $this->freeline_sales_transaction_model->update($id, $data, $items, $point);
	...
	function detail_list_loader($transaction_id=0)
	{
		...
		$data = $this->freeline_sales_transaction_model->detail_list_loader($transaction_id);
		$sort_id = 0;
	...
	{		
		...
		$this->render->add_form('app/freeline_sales_transaction/transient_form', $data);
	...
	{		
		...
		$check_stock = $this->freeline_sales_transaction_model->check_stock($product_stock_id);
		
		$get_data_product = $this->freeline_sales_transaction_model->get_data_product($product_stock_id);
	...
	function load_product_stock()
	{
		...
		$query = $this->freeline_sales_transaction_model->load_product_stock($id);
		$data = array();
		...
		foreach($query->result_array() as $row)
		{
			...
			$data['price'] = $row['freeline_price'];
			$data['qty'] = 1;
			$data['total'] = $row['freeline_price'];
		}
	...
	if($id){
	   ...
	   $result = $this->freeline_sales_transaction_model->read_id($id);
		...
		$data_detail = $this->freeline_sales_transaction_model->get_data_detail($id);
	   
	   $this->global_model->create_report('freeline_sales_transaction', 'report/freeline_sales_transaction.php', $data, $data_detail, 'header.php');
	}
	}
