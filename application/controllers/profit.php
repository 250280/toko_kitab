...
{	
	function __construct()
	{
		...
		$this->access->set_module('accounting.profit');
	...
	function index()
	{
		...
		$this->render->add_view('app/profit/list');
		$this->render->build('Laba Kotor');
		
		$this->render->show('Laba Kotor');
	}
	function table_controller()
	{
		$data = $this->profit_model->list_controller();
		send_json($data); 
	}
	...
	function form($id = 0)
	{
		...
		if ($id == 0) {
			...
			$result = $this->profit_model->read_id($id);
		...
		$list_nama_pendapatan = $this->profit_model->get_list_nama_pendapatan();
		$data_pendapatan = $this->profit_model->get_data_pendapatan($id);
			
		$this->render->add_form('app/profit/form', $data);
		$this->render->build('Laba Kotor');
		$this->render->add_view('app/profit/transient_list', array('list' => $list_nama_pendapatan, 'data_pendapatan' => $data_pendapatan));
		$this->render->build('Detail Laba Kotor');
		if($id){
	...
	function form_detail($id = 0)
	{
		$data = array();
			...
			$list = $this->profit_model->get_list_nama_pendapatan();
		...
		$this->render->add_form('app/profit/form_detail', $data);
		$this->render->build('Laba Kotor');
		$this->render->show('Laba Kotor');
	}
	...
	function form_detail_edit($id = 0)
	{
		$data = array();
				...
				$result = $this->profit_model->read_id_item($id);
				if($result){
					$data = $result;
					$data['row_id'] = $id;
					$data['profit_date'] = date('d/m/Y', strtotime($data['profit_date']));
				}
				
				
			$list = $this->profit_model->get_list_nama_pendapatan_edit($id);
				
				$no = 0;
				foreach($list as $item): 
          			
					$data['subject_id'][$no] = $item['profit_item_id'];
					$data['subject_name'][$no] = $item['coa_name'];
					$data['subject_value'][$no] = $item['profit_item_value'];
    				$no++;
		...
		$this->render->add_form('app/profit/form_detail_edit', $data);
		$this->render->build('Laba Kotor');
		$this->render->show('Laba Kotor');
	}
	...
	function form_action($is_delete = 0) // jika 0, berarti insert atau update, bila 1 berarti delete
	{
		...
		if($is_delete)
		{
			...
			$is_process_error = $this->profit_model->delete($id);
		...
		$this->form_validation->set_rules('i_profit_date','Tanggal','trim|required|valid_date|sql_date');
		for($i=0; $i<$no; $i++){
		...
		$data['profit_date'] 					= $this->input->post('i_profit_date');
			...
			{ 
				...
				$data_is_exist = $this->profit_model->data_is_exist($data['period_id'], $data['profit_date']);
				if($data_is_exist){
					send_json_error("Simpan gagal. Data penjualan untuk tanggal ". $data['profit_date'] ." sudah ada");
				}
				
				for($u=0; $u<$no; $u++){
					$items['coa_id'][$u] = $this->input->post('i_subject_id'.$u);
					$items['profit_item_value'][$u] = $this->input->post('i_subject_value'.$u);
				}
			
				$error = $this->profit_model->create($data, $items, $no);
				send_json_action($error, "Data telah ditambah", "Data gagal ditambah");
			}
			else // id disebutkan, lakukan proses UPDATE
			{
				for($u=0; $u<$no; $u++){
					$items['profit_item_id'][$u] = $this->input->post('i_subject_id'.$u);
					$items['profit_item_value'][$u] = $this->input->post('i_subject_value'.$u);
				}
				
				$error = $this->profit_model->update($id, $data, $items, $no);
	...
	function detail_list_loader($period_id=0)
	{
		...
		$data = $this->profit_model->detail_list_loader($period_id);
		$sort_id = 0;
	...
	{		
		...
		$this->render->add_form('app/profit/transient_form', $data);
	...
	{		
		...
		$period_name 	= $this->profit_model->get_period_name($period);
		$market_name 	= $this->profit_model->get_market_name($market);
		$coa_name	 	= $this->profit_model->get_coa_name($coa);
		$coa_hierarchy	= $this->profit_model->get_coa_hierarchy($coa);
