...
class product_type extends CI_Controller 
{
	function __construct()
	{
		...
		$this->access->set_module('master.product_type');
	...
	function index()
	{
		...
		$this->render->add_view('app/product_type/list');
		$this->render->build('Data Tipe Produk');
	//	$this->access->generate_log_view();
		$this->render->show('Tipe Produk');
		
	}
	function list_loader()
	{
		$data = $this->product_type_model->list_loader();
	...
	function form() // jika id tidak diisi maka dianggap create, else dianggap edit
	{
		...
		if ($id == 0) {
			...
			$data['row_id'] = '';
			$data['i_name'] = '';//format_code('product_categories','product_type_code','',3);
			$data['i_description'] = '';
			
		} else {
			
			// FORM UPDATE - ambil data yang diedit kemudian tampilkan dalam form			
			$result = $this->product_type_model->read_id($id);
			
			if ($result) // cek dulu apakah data ditemukan 
			{
				$data['row_id'] = $result['product_type_id'];
				$data['i_name'] = $result['product_type_name'];
				$data['i_description'] = $result['product_type_description'];		
			}
		}
		
		$this->render->add_form('app/product_type/form', $data);
	...
	function form_action($is_delete = 0) // jika 0, berarti insert atau update, bila 1 berarti delete
	{
		...
		{		
			$id = $this->input->post('row_id');
			$is_process_error = $this->product_type_model->delete($id);
		...
		{
			$data['product_type_name'] = $this->input->post('i_name');
			$data['product_type_description'] = $this->input->post('i_description');
			$error = $this->product_type_model->create($data);
		...
		else // id disebutkan, lakukan proses UPDATE
		{
			...
			$error = $this->product_type_model->update($id, $data);
