...
{	
	function __construct()
	{
		...
		$this->access->set_module('transaction.purchase');
	...
	function index()
	{
		$this->render->add_view('app/purchase/list');
		$this->render->build('Transaksi Pembelian');
		$this->render->show('Transaksi pembelian');
		
	}
	function table_controller()
	{
		$data = $this->purchase_model->list_controller();
		send_json($data); 
	}
	...
	function form($id = 0)
	{
		...
		if ($id == 0) {
			...
			$result = $this->purchase_model->read_id($id);
		...
		$this->render->add_form('app/purchase/form', $data);
		$this->render->build('Transaksi Pembelian');
		
		$this->render->add_view('app/purchase/transient_list');
		$this->render->build('Data Produk');
		
		//$this->render->add_view('app/purchase/form_end', $data);
	...
	function form_action($is_delete = 0) // jika 0, berarti insert atau update, bila 1 berarti delete
	{
		...
		if($is_delete)
		{
			...
			$is_process_error = $this->purchase_model->delete($id);
		...
		$this->form_validation->set_rules('i_vendor_id','Vendor','trim|required|integer');
		...
		$data['subject_id']					= ($this->input->post('i_vendor_id')) ? $this->input->post('i_vendor_id') : 0;
		...
		{
			$get_purchase_price = $this->purchase_model->get_purchase_price($list_product_id[$key]);
		...
		{
			$error = $this->purchase_model->update($id, $data, $items);
	...
	function detail_list_loader($transaction_id=0)
	{
		...
		$data = $this->purchase_model->detail_list_loader($transaction_id);
		$sort_id = 0;
	...
	function load_product_stock()
	{
		...
		$query = $this->purchase_model->load_product_stock($id);
		$data = array();
		...
		foreach($query->result_array() as $row)
		{
			...
			$data['product_purchase_price'] = $row['product_purchase_price'];
		}
		send_json_message('Product', $data);
	}
	...
	if($id){
	   ...
	   $result = $this->purchase_model->read_id($id);
		...
		$data_detail = $this->purchase_model->get_data_detail($id);
