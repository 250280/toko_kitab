...
class Gl extends CI_Controller 
{
	...
	{	
		...
		$this->render->build('Daftar Jurnal');
		$this->render->show('Jurnal');
	...
	function form($id=0)
	{
		...
		if($id == 0)
		{
			...
			$data['transaction_code'] 			= format_code('transactions_sl','transaction_code','JU',7);
		...
		$this->render->build('Detail Jurnal');
		$this->render->show('Jurnal');			
	}// end of function 
	...
	function gl_form_action($is_delete=0)
	{
		...
		{	
			...
			if($check_approved){
				...
				$is_process_error = $this->gl_model->delete($id);
			}
		...
		if(!$id)
		{
			//$prefix = $this->gl_model->create_code_prefix();		
