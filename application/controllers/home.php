...
#		exit;
	...
	//	$this->_setup_walker(1);
		...
		if (!$redir) redirect('dashboard'); else { 
			if(!isset($redir['redir_url']))redirect('dashboard');
...
#		debug($depth);
		...
		$str = '';
		$this->load->model('menu_model');
		$total_expired = $this->menu_model->get_expired_stock();
		

		if (($this->raw[$p]['visible'] == 1)) {
			
			$icon = ($this->raw[$p]['icon']) ? $this->raw[$p]['icon'] : "fa-chevron-circle-right";
			
			$str .= '<li> <i class="fa '.$icon.'"></i> <span><a href="' . site_url($this->raw[$p]['url']) . '">' . $this->raw[$p]['name'] . '</a></span>';
...
//print_r($this->root);exit;
		if (isset($this->root[$p])) 
		{
			$item = '';
			$no = 0;
			foreach($this->root[$p] as $v) $item .= $this->_visible_walker($v, $depth + 1); $no++;
			//if($p != 1) $str .= '</li>';
			if (!empty($item)) 
			{
