...
class Render
{
	...
	function build_dashboard($data_title, $frame = "win2", $ico = '')
	{
		...
		$data['title2'] = ucwords($data_title[1]);
...
where a.menu_url like '$url' or a.menu_url like '$seg1'");		
	...
	function user_info()
	{
		...
		if ($ci->access->is_logged()) 
		{
			...
			$params['employee'] = $ci->access->info['employee_name'];// . "(" . $ci->access->info['employee_id'] . ")";
			$params['employee_pic'] = $this->get_employee_pic($params['user_id']);
			$params['employee_pic'] = ($params['employee_pic']) ? $params['employee_pic'] : "default.jpg";
			//$params['branch'] = $ci->access->branch_name . "(" .  $ci->access->branch_id . ")";
			$params['group_name'] = $ci->access->info['group_name'];
			
			$expired_stock = $this->get_expired_stock();
			$params['expired_stock'] = $expired_stock;
	...
	{
		$ci = & get_instance();
		$sql = "select employee_pic from users a
